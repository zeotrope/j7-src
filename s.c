...
static I probe(a,symb,is)A a,symb;C is;{A t;C*av;I c,i,k,m;SY*tab;
 av=CAV(a); tab=SYAV(symb); m=AN(symb);
...
static F1(ex){A*wv,z;B*zv;
 ...
 wv=AAV(w); zv=BAV(z); DO(AN(w),RE(*zv++=ex1(*wv++)));
 R z;
}
...
F1(ncx){A t,y;
 F1RANK(0,ncx,0);
 R sc(BOX&AT(w) && (t=*AAV(w)) && AN(t) && 1>=AR(t) && AT(t)&CHAR+NAME &&
...
static B nlmc(w)A w;{SY*e=SYAV(w); DO(AN(w), if(e->name&&e->val)R 1; ++e); R 0;}
     /* 1 if symbol table w is non-empty */

static F1(nlm){A x,y,*yv;I k=0;SY*e=SYAV(w);
 GA(y,BOX,AN(w),1,0); yv=AAV(y);
