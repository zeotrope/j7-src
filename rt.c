...
static F2(troot){A t,x;B b;C*u,*v;I j,k,m,n,*s;
 RZ(a&&w);
 m=AN(a); u=CAV(a); b=!m||'0'<=*u&&*u<='9';
 GA(x,CHAR,b?1:4+m,1,0); v=CAV(x);
...
static F1(tleaf){A t,*x,z;C d[2],*v;I n,*s;
 RZ(w);
 GA(z,BOX,1,1,0); x=AAV(z);
 if(CHAR&AT(w)&&1>=AR(w)){
  n=AN(w);
  GA(t,CHAR,2+n,2,0); s=AS(t); *s=1; *++s=2+n;
  v=CAV(t); *v=xdash; *(v+1)=' '; MC(2+v,AV(w),n);
  *x=t;
 }else{
...
static F1(connect){A*wv,x,y,z;B b,d;C c,*u,*xv,*yv,*zv;I e,i,j,m,n,p,q;
 ...
 GA(z,CHAR,m*e,2,AS(y)); *(1+AS(z))=e; zv=CAV(z);
 for(i=0;i<n;++i){
