...
#define ENGAP(j,r,s,exp) do{C b;I k=1;I p=j;I*sr=s+r-2; \
                            DO((r-1)*!!p, k*=*(sr-i); b=!(p%k); exp); }while(0)
...
static F1(enframe){A d,*v,x,y,z;C*s,*t,*zv;I ht,j,k,m,n,p,q,r,wd,xn,*xv,yn,*yv;
 ...
 GA(z,CHAR,m*p,r,AS(w)); *(AS(z)+r-2)=ht; *(AS(z)+r-1)=wd; zv=CAV(z);
 if(!n)R z;
...
F2(thorn2){PROLOG;A z;C buf[NTH2],p[15],*wv,*x;D*av,e,y;I an,i,k,m=0,n,wd,wk,wn,wt,zn;
 ...
 RZ(a=cvt(FL,a));
 av=DAV(a); wv=CAV(w);
 zn=20*n; GA(z,CHAR,zn,1,0); x=CAV(z);
 for(i=0;i<n;++i){
  if(!i||1<an){
