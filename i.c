...
C jinit2(argc,argv)int argc;C**argv;{A*v;B b=1,n=0;C p[256]="profile.js",*s;D*d;
 ...
 sesminit();
 symbinit();
 GGA(alp,CHAR,NALP,1); s=CAV(alp); DO(NALP, *s++=i); ps[psptr[(UC)CALP]].f1=(AF)alp;
 GGA(mtv, BOOL,0,1);
 GGA(mtc, CHAR,0,1);
 GGA(zero,BOOL,1,0); *BAV(zero)=0;
 GGA(one, BOOL,1,0); *BAV(one )=1;
 ...
 MC(&inf,XINF,(size_t)sizeof(D)); GGA(ainf,FL,1,0); *DAV(ainf)=inf;
 MC(&nan,XNAN,(size_t)sizeof(D));
 GA(qevm,BOX,1+NEVM,1,0); v=AAV(qevm);
